...
6. **Название задачи** для верного понимания контекста где хранить информацию.
...
- Финальные артефакты (review результаты) пиши на русском языке
...
7. **Следи за размером контекста** — периодически вызывай команду /compress для суммаризации контекста, чтобы избежать его переполнения
