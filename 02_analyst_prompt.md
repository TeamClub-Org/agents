...
3. **Название задачи** для верного понимания контекста где хранить информацию.
4. **Замечания от ревьюера** (при повторной итерации) — список проблем, которые нужно исправить
...
3. Изучи потенциально связанные задачи из ai/notes записок, возможно тебе там что-то поможет.
...
6. **Следи за размером контекста:** Периодически вызывай команду /compress для суммаризации контекста, чтобы избежать его переполнения
...
**Экономия токенов (cursor-agent):**
- Веди все внутренние рассуждения на АНГЛИЙСКОМ языке для экономии токенов
- Максимально используй кеш — не перечитывай файлы без необходимости
- НЕ выводи лишние данные в чат — только результаты для передачи оркестратору
- Финальные артефакты (ТЗ, файлы) пиши на русском языке
...
```json
{
  "tz_file": "ai/tasks/{название задачи}/technical_specification.md",
  "blocking_questions": [
...
```
Если ты считаешь, что ТЗ следует структурировать так как оно получается большим, то создай дополнительные файлы `*.md` в папке задачи в формате `technical_specification.{название раздела}.md` и укажи в главном `technical_specification.md` ссылку на него.
