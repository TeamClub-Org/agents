...
## ЗАПУСК СУБ-АГЕНТОВ (cursor-agent CLI)

### Основной формат команды

cursor-agent -f --model {модель} -p "{промпт}" --approve-mcps 2>&1 | tee {лог-файл}

### Ключевые флаги

| Флаг | Назначение |
|------|------------|
| `-f, --force` | Автоматически одобрять команды |
| `-p, --print` | Вывод в консоль (для скриптов) |
| `--model <model>` | Модель: opus-4.5, composer-1 |
| `--approve-mcps` | Автоматически одобрять MCP серверы |
| `--output-format stream-json` | Стриминг JSON вывода |
| `--resume <chatId>` | Возобновить сессию |

### Выбор моделей по ролям

| Роль | Модель | Причина |
|------|--------|---------|
| Аналитик | opus-4.5 | Глубокий анализ требований |
| Архитектор | opus-4.5 | Сложные архитектурные решения |
| Планировщик | opus-4.5 | Декомпозиция и планирование |
| Ревьюеры (ТЗ, архитектуры, плана, кода) | composer-1 | Проверка и валидация |
| Разработчик | composer-1 | Написание кода |

### Формат промпта

Промпт формируется из двух частей:
1. Содержимое файла роли (agents/02_*.md ... agents/09_*.md)
2. Входные данные согласно описанию роли
...
### Правила экранирования промпта

1. **НЕ передавай большие файлы целиком в промпте** — указывай пути к файлам
2. **Записывай сложные промпты в файл** и читай через $(cat файл.md)
3. **Избегай специальных символов в промпте** — $, `, ", ' требуют экранирования
4. **Используй | tee для логирования** — результат сохраняется в файл

### Примеры запуска

# Аналитик
cursor-agent -f --model opus-4.5 -p "$(cat agents/02_analyst.md)

---
# ВХОДНЫЕ ДАННЫЕ
Файл задачи: ai/tasks/TC-17/analyst_input.md
Описание проекта: ai/project.md
" --approve-mcps 2>&1 | tee ai/tasks/TC-17/analyst_log.txt

# Ревьюер плана (минимальный промпт — пути к файлам)
cursor-agent -f --model composer-1 -p "$(cat agents/07_agent_plan_reviewer.md)

---
# ВХОДНЫЕ ДАННЫЕ
Файлы для ревью:
- ai/tasks/TC-17/technical_specification.md
- ai/tasks/TC-17/plan.md  
- ai/tasks/TC-17/tasks/ (папка с задачами)
Название задачи: TC-17
" --approve-mcps 2>&1 | tee ai/tasks/TC-17/plan_review_log.txt

### Обработка результатов

1. **Проверяй созданные файлы** — агенты создают артефакты в ai/tasks/{task}/
2. **Читай логи при ошибках** — файлы *_log.txt содержат полный вывод
3. **Возобновляй прерванные сессии** — cursor-agent --resume="chat-id"

### Типичные проблемы

| Проблема | Решение |
|----------|---------|
| Timeout команды | Запусти в background: is_background=true, проверяй через терминал |
| Агент не видит файлы | Указывай абсолютные пути или относительные от корня проекта |
| Промпт слишком длинный | Запиши в файл, читай через $(cat файл.md) |
| Специальные символы | Экранируй или используй файл с промптом |

---

## ОБЯЗАТЕЛЬНЫЙ МОНИТОРИНГ СУБ-АГЕНТОВ

### ⚠️ КРИТИЧЕСКОЕ ПРАВИЛО

**НИКОГДА не переходи к следующему шагу, пока суб-агент не завершил работу!**

После запуска суб-агента ты ОБЯЗАН:
1. Дождаться завершения агента
2. Проверить результаты его работы
3. Только потом переходить к следующему этапу

### Алгоритм мониторинга

1. **Запусти агента в background** (is_background=true)
2. **Периодически проверяй терминал** — читай файл terminals/{id}.txt
3. **Ищи признаки завершения:**
   - Строка "To resume this session: cursor-agent --resume=..." означает ЗАВЕРШЕНИЕ
   - Отсутствие `active_command:` в заголовке терминала означает ЗАВЕРШЕНИЕ
   - Наличие `active_command:` означает агент ЕЩЁ РАБОТАЕТ
4. **После завершения проверь артефакты:**
   - Созданные/изменённые файлы
   - Лог-файл (*_log.txt)
   - Ожидаемые результаты согласно роли агента

### Формат проверки терминала

Читай файл терминала: `/Users/{user}/.cursor/projects/.../terminals/{id}.txt`

Заголовок терминала содержит:
```
---
pid: 12345
cwd: /path/to/project
active_command: cursor-agent ...  <-- АГЕНТ ЕЩЁ РАБОТАЕТ
---
```

или (если агент завершился):
```
---
pid: 12345
cwd: /path/to/project
last_command: cursor-agent ...
last_exit_code: 0  <-- АГЕНТ ЗАВЕРШИЛСЯ
---
```
...
- Последующие проверки: каждые 30-60 секунд
- Таймаут: 5 минут (после этого считать зависшим)

### Пример мониторинга

```
1. Запускаю агента в background → терминал 11
2. Жду 30 секунд
3. Читаю terminals/11.txt
4. Вижу active_command → агент работает, жду ещё
5. Читаю terminals/11.txt снова
6. Вижу "To resume this session" → агент завершился
7. Проверяю результаты: созданные файлы, лог
8. Перехожу к следующему шагу
```
...
МОНИТОРИНГ ЗАВИСШИХ АГЕНТОВ:
- Максимальное время выполнения задачи суб-агентом: 5 минут
- Каждые 60 секунд проверяй статус работающего агента
- Если агент не отвечает более 3 минут — считай его зависшим
- При зависании:
  1. Принудительно останови зависший процесс (kill команды/тестов)
  2. Запиши причину зависания в лог
  3. Повтори запуск агента с теми же входными данными (максимум 2 попытки)
  4. Если повторный запуск также завис — подключи пользователя
- Типичные причины зависания: бесконечные тесты, ожидание ввода, сетевые таймауты
- При перезапуске агента добавь инструкцию: "Избегай долгих операций. Если тесты выполняются более 60 секунд — прерви их и сообщи об этом."

СТРУКТУРА ПРОЦЕССА:
1. Анализ (Аналитик → Ревьюер ТЗ)
2. Архитектура (Архитектор → Ревьюер архитектуры)
3. Планирование (Планировщик → Ревьюер плана)
4. Проверка плана и архитектуры человеком.
...
Не запускай этап "Разработка" до того как пользователь подтвердит намерение перейти к нему.
```
...
### ОБЯЗАТЕЛЬНО: Мониторинг зависания процессов

Каждый раз, когда задача или действие требуют длительного выполнения (например, запуск тестов, сборка, ожидание пользовательского ввода, сетевые операции):

1. Следи за таймингом процесса:
   - Каждые 60 секунд проверяй, завершилась ли операция.
   - Если процесс длится более 3 минут, считай его зависшим.
2. В случае зависания:
   - Принудительно останови выполнение (kill процесс/отмени операцию).
   - Зафиксируй причину зависания в лог.
   - Один раз перезапусти операцию с теми же входными данными.
   - Если повтор зависает — приостанови этап, подключи пользователя и сообщи о проблеме.
3. В инструкциях к перезапущенному агенту укажи: "Избегай долгих операций. Если тесты или сборка длятся более 60 секунд — прерви их и сообщи об этом."

Сабагент всегда обязан выполнять данный мониторинг при любой длительной операции на этом этапе!
...
ТВОЯ ЗАДАЧА:
Подготовить итоговый отчёт для пользователя и обновить описание задач в таск трекере.
...
4. Обнови соответствующие задачи в таск-трекере через MCP Trello
5. Создай notes для каждой созданной задачи в markdown формате в папке ai/notes/{название задачи}.

ФОРМАТ NOTE ДЛЯ ЗАДАЧИ:

##{Название задачи}
## Теги
{теги, которые описывают основные затронутые части системы, или тип задачи}
## Описание
{краткое описание}
...
## Промпт 16: Принятие решения о дальнейшей разработке

```
КОНТЕКСТ: Получены ответы на все вопросы от сабагентов, ТЗ создано, Архитектура создана и утверждена. Нет критических замечаний.
...
ТВОЯ ЗАДАЧА:
Остановить процесс и передать управление пользователю. Он принимает решение о дальнейшем продолжении.

ДЕЙСТВИЯ:
1. Сохрани текущее состояние процесса
2. Сформируй понятное сообщение для пользователя
3. Дождись ответа от пользователя
4.1 В случае положительного решения создай задачи в таск трекере Trello через MCP который доступен для этого проекта.
4.2 Прекрати процесс в случае отказа

ФОРМАТ СООБЩЕНИЯ ПОЛЬЗОВАТЕЛЮ:

⚠️ ПРОЦЕСС ОСТАНОВЛЕН: Требуется подтверждение

Следует принять решение о дальнейшем продолжении разработки.

Контекст:
{краткое описание ситуации}

Задачи, которые будут созданы после подтверждения:
{список задач с их названием, кратким описаниеми и список связей между собой}

ПРАВИЛА СОЗДАНИЯ ЗАДАЧ
Изначально на вход тебе приходит задача из таск-трекера. Это эпик, который должен быть разделен на тематические задачи: Анализ, Архитектура, Разработка.

На этом этапе задача по Аналитике и Архитектуре формально уже завершены, поэтому их нужно создать, подцепить к главной задаче-эпику, заполнить описанием и артефактами.

Задачи по разработке следует так же создать в таск трекере с правильным статусом, задать описание и связать между собой.

Не мельчи задачи, ты можешь некоторые из них формально объеденить.

ПОСЛЕ ПОЛУЧЕНИЯ ПОЛОЖИТЕЛЬНОГО ОТВЕТА:
1. Создай задачи в таск трекере через MCP, загружая в них все артефакты созданные агентами
2. Возобнови процесс с того же места
3. Отслеживай процесс разработки

ПОСЛЕ ПОЛУЧЕНИЯ ОТРИЦАТЕЛЬНОГО ОТВЕТА:
1. Останови работу всех агентов
```