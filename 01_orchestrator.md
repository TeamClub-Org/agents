...
- Следи за размером потребляемого контекста. Периодически вызывай команду /compress для суммаризации контекста, чтобы избежать его переполнения
- ВСЕГДА пиши на РУССКОМ языке при общении с пользователем и в отчётах
