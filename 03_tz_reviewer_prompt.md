...
4. **Название задачи** для верного понимания контекста где хранить информацию.
...
```json
{
  "review_file": "ai/tasks/{название задачи}/tz_review.md",
...
5. **Следи за размером контекста:** Периодически вызывай команду /compress для суммаризации контекста, чтобы избежать его переполнения
...
- Финальные артефакты (review файлы) пиши на русском языке
