...
- ✅ Следить за размером потребляемого контекста и периодически вызывать команду /compress для суммаризации контекста

### Экономия токенов (cursor-agent)
- **Оркестратор:** Пишет на РУССКОМ языке при общении с пользователем
- **Суб-агенты (Аналитик, Архитектор, Планировщик, Разработчик, Ревьюеры):**
  - Ведут внутренние рассуждения на АНГЛИЙСКОМ языке для экономии токенов
  - Максимально используют кеш — не перечитывают файлы без необходимости
  - НЕ выводят лишние данные в чат — только результаты для передачи оркестратору
  - Финальные артефакты (документы, отчёты) пишут на русском языке
